...
TitleScreen:
	...
	move.w	#$8720,(a6)		; Background palette/color: 3/0
	...
	lea	(ArtNem_TitleScoreRushBG).l,a0
	bsr.w	NemDec
	...
	; Decompress the tiny Sonic 2 logo plane map...
	...
	; ...and send it to VRAM.
	lea	(Chunk_Table).l,a1
	move.l	#vdpComm(VRAM_TtlScr_Plane_A_Name_Table+(7*$80)+(13*2),VRAM,WRITE),d0
	moveq	#14-1,d1 ; Width
	moveq	#4-1,d2 ; Height
...
TitleScreen_WaitLoop:
	...
	; Decompress the title screen background plane map...
	lea	(Chunk_Table).l,a1
	lea	(MapEng_TitleScoreRushBG).l,a0
	...
	; ...and send it to VRAM.
	lea	(Chunk_Table).l,a1
	move.l	#vdpComm(VRAM_TtlScr_Plane_A_Name_Table+(11*$80)+(10*2),VRAM,WRITE),d0
	moveq	#20-1,d1 ; Width
	moveq	#9-1,d2 ; Height
	jsrto	PlaneMapToVRAM_H40, PlaneMapToVRAM_H40	
...
-	move.l	(a1)+,(a2)+
	...
	moveq	#PalID_TitleScoreRushBG,d0
...
; -	move.w	#-1,(a1)+
	...
	move.w	#MusID_Title,d0
	jsr	(PlayMusic).l	; Play transformation sound effect.		
	bsr.w	Pal_FadeFromWhite

; loc_3C14:
TitleScreen_Loop:
	move.b	#VintID_Unused6,(Vint_routine).w
	bsr.w	WaitForVint

	; jsr	(RunObjects).l
	addq.w	#1,(Demo_Time_left).w
